...
export async function checkQuotationService(): Promise<void> {
  ...
  try {
    ...
    if (payload.quotationSessionValidationConfigured !== true) {
      // ไม่เขียนชื่อคีย์ฝั่งเซิร์ฟเวอร์เต็ม ๆ ไว้ในโค้ดหน้าบ้าน
      // เพราะ securityContract.test.ts ห้ามคำนั้นโผล่ในซอร์สที่ถูก build ไปให้ผู้ใช้
      // ชื่อ Script Properties ที่ต้องตั้ง ดูได้ใน docs/RUNBOOK.md หัวข้อ Apps Script
      throw new Error('Apps Script ยังไม่ได้ตั้ง Script Properties สำหรับตรวจ session (SUPABASE_URL และคีย์ฝั่งเซิร์ฟเวอร์) ดูขั้นตอนใน docs/RUNBOOK.md');
    }
  } catch (error) {
