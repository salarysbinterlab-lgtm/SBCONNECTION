...
describe('security contract', () => {
  ...
  it('ทุกชื่อใน allowlist ต้องถูก grant ให้ anon ใน sql/17 และไม่มีชื่อเกิน', () => {
  ...
  it('ไม่มีไฟล์ SQL ไหน grant ฟังก์ชันนอก allowlist ให้ anon', () => {
    ...
    for (const path of sqlFiles) {
      ...
      if (!/1[7-9]_|20_/.test(path)) continue;
  ...
  it('ฟังก์ชันของระบบลืมรหัสผ่านต้องไม่เปิดให้ anon', () => {
  ...
  it('sql/17 ต้องมีคำสั่งปิดสิทธิ์แบบครอบทั้ง schema', () => {
    const lockdown = readFileSync(join(root, 'sql', '17_SECURITY_HARDENING_AND_FIXES.sql'), 'utf8');
  ...
  it('ฟังก์ชันภายในต้องถูก revoke อย่างชัดเจนใน sql/17', () => {
    const lockdown = readFileSync(join(root, 'sql', '17_SECURITY_HARDENING_AND_FIXES.sql'), 'utf8');
    for (const fn of [
