    ...
    try {
      ...
      if (isContent) {
        payload = {
          ...
          link_url: String(editingRow.link_url || '').trim(),
          // ฐานข้อมูลเก็บ stock เป็น integer not null ไม่มีค่า "ไม่จำกัด"
          // ถ้าส่ง null ไป จะกลายเป็น 0 = ของหมดทันที จึงบังคับให้เป็นตัวเลขเสมอ
          stock: Math.max(0, Number(editingRow.stock ?? 0) || 0),
                  ...
                  <>
                    ...
                    {activeModule === 'missions' && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 rounded-2xl border p-3" style={{ borderColor: thm.border + '45' }}>
                        ...
                        <div className="sm:col-span-2">
                          <label className="text-[10px] font-black opacity-45 block mb-1">
                            {lang === 'th' ? 'ลิงก์ภารกิจ เช่น ลิงก์โพสต์ที่ให้ไปกดไลก์ (ไม่บังคับ)' : 'Mission link, e.g. a post to like (optional)'}
                          </label>
                          <input value={editingRow.link_url || ''}
                            onChange={e => setEditingRow({ ...editingRow, link_url: e.target.value })}
                            className="w-full bg-slate-950/5 border rounded-2xl h-11 px-4 text-xs font-bold outline-none"
                            placeholder="https://www.facebook.com/..."
                            style={{ color: textColor, borderColor: thm.border + '60' }} />
                          <p className="mt-1 text-[10px] font-bold opacity-45 leading-relaxed">
                            {lang === 'th'
                              ? 'ต้องขึ้นต้นด้วย http:// หรือ https:// ระบบเช็คได้แค่ว่ากดเปิดลิงก์แล้ว เช็คไม่ได้ว่ากดไลก์จริง ถ้าต้องการหลักฐานให้ติ๊ก "ต้องแนบหลักฐาน" ด้วย'
                              : 'Must start with http:// or https://. The system can only confirm the link was opened, not that it was liked. Tick "Evidence required" for proof.'}
                          </p>
                        </div>
                      </div>
                    )}
                      ...
                      {activeModule === 'rewards' && (
                        <div>
                          <label className="text-[10px] font-black opacity-45 block mb-1">Stock count (จำนวนที่แลกได้ 0 = ปิดการแลก)</label>
                          <input type="number" min={0} required value={editingRow.stock ?? 0}
                            onChange={e => setEditingRow({ ...editingRow, stock: Math.max(0, Number(e.target.value) || 0) })}
                            className="w-full bg-slate-950/5 border rounded-2xl h-11 px-4 text-xs font-bold outline-none"
                            placeholder="0"
                            style={{ color: textColor, borderColor: thm.border + '60' }} />
                        </div>
